...
contract ScoresAndQuotas {
    ...
    function strToUint(bytes memory b) internal pure returns (uint result) {
        uint i = 0;
        uint tr = 0;
        result = 0;
        while(uint(uint8(b[b.length - tr - 1])) == 0x00) {
            tr++;
        }
        for (;i < b.length - tr; i++) { 
    ...
    function fulfill(bytes32 _data) external{
        ...
        if(keccak256(abi.encodePacked(_data[0])) == keccak256(abi.encodePacked("s"))){
            GameScore storage gameScore = gameScores[uint16(strToUint(bytes(split[3])))];
            gameScore.home = uint8(strToUint(bytes(split[1])));
            gameScore.away = uint8(strToUint(bytes(split[2])));
        }else{
            Quotas storage quotas = gameQuotas[uint16(strToUint(bytes(split[4])))];
